...
public class BinaryTree {
	...
	protected int _lNodeCount; //Tracks the number of nodes in the tree, also tracks the number of unique words
	protected int _lWordCount; //Tracks the number of words in the file
	protected WordPosPair _wpMostCommon; //Tracks the most often occurring word
	...
	public BinaryTree(Object oElem) {
		_nRoot=new BTNode(oElem);
		_lHeight=1;
		_lNodeCount=1;
	}
	//Tree with root
	public BinaryTree(BTNode nRoot){
		//Make the head and tail point to nothing and make the length 0
		_nRoot=nRoot;
		_lHeight=CalcHeight();
	...
	//Setter
	public void SetRoot(BTNode nRoot){
	...
	public boolean AddString(String sData) {
		//Start at the root
		...
		while (true) {
			...
			if (sCurr.compareTo(sData)<0) {
				...
				if (nParent.GetRight()!=null) {
					...
					//Make a new node
					BTNode nNew = new BTNode(sData,nParent);
					//Add the new node to the tree
					nParent.SetRight(nNew);
					//Up the node count
					_lNodeCount+=1;
					//If we have traversed deep enough, we could have just increased the height, in which case, _lHeight must be updated
					if (iDepth>_lHeight) {
						_lHeight = iDepth;
					}
					//The string was not already in the tree, return false
	...
	public boolean AddWordPos(String sData, int lPos) {
		//Start at the root
		BTNode nParent = GetRoot();
		WordPosPair wCurr;
		String sCurr;
		...
		while (true) {
			...
			if (sCurr.compareTo(sData)<0) {
				...
				if (nParent.GetRight()!=null) {
					...
					//Make a new node
					BTNode nNew = new BTNode(new WordPosPair(sData,lPos),nParent);
	...
	//Given a word as input, this function finds it in the tree, and returns the WordPosPair associated to that word
	...
	public int CalcHeight(BTNode nHead) {
		if (nHead==null) {
	...
	public int CalcNodeCount(BTNode nHead) {
		if (nHead==null) {
			//If nHead does not exist, then there is just as much tree 
			// as there is spoon.
			// Reference to The Matrix. Watch it before calling yourself 
