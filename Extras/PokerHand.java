...
public class PokerHand {
	...
	private int[] _iCards; //_iCards[i]/4 = value of card, _iCards[i]%4 = suit
	...
	public int stoc(String sCard){
		char cVal=sCard.charAt(0);
		char cSuit=sCard.charAt(1);
		int iCard;
		if (cSuit=='S') iCard=SUIT_SPADES;
		else if (cSuit=='H') iCard=SUIT_HEARTS;
		else if (cSuit=='C') iCard=SUIT_CLUBS;
		else iCard=SUIT_DIAMONDS;
		
		if (cVal=='2') iCard+=2*4;
		else if (cVal=='3') iCard+=3*4;
		else if (cVal=='4') iCard+=4*4;
		else if (cVal=='5') iCard+=5*4;
		else if (cVal=='6') iCard+=6*4;
		else if (cVal=='7') iCard+=7*4;
		else if (cVal=='8') iCard+=8*4;
		else if (cVal=='9') iCard+=9*4;
		else if (cVal=='T') iCard+=10*4;
		else if (cVal=='J') iCard+=CARD_JACK*4;
		else if (cVal=='Q') iCard+=CARD_QUEEN*4;
		else if (cVal=='K') iCard+=CARD_KING*4;
		else if (cVal=='A') iCard+=CARD_ACE*4;
		
		return iCard;
	}
	
	static public int compare(PokerHand ph1,PokerHand ph2){
		int iVal1;
		int iVal2;
	...
	static public int HighCard(PokerHand ph){
		int iMax=0;
	...
	static public int NextHighCard(PokerHand ph,int iCard){
		int iMax=0;
	...
	static public int HasPair(PokerHand ph){
		for(int i=0;i<4;i++){
			for(int k=i+1;k<5;k++){
	...
	static public int HasTwoPair(PokerHand ph) {
		int iFirstPair=HasPair(ph);
		for(int j=0;j<4;j++){
	...
	static public int HasThree(PokerHand ph) {
		for(int i=0;i<3;i++){
			for(int k=i+1;k<4;k++){
				if(ph.GetCard(i)/4==ph.GetCard(k)/4) {
					for(int j=k+1;j<5;j++){
	...
	static public int HasStraight(PokerHand ph){
		int iMax=HighCard(ph);
		for(int i=0;i<5;i++){
	...
	static public int HasFlush(PokerHand ph){
	...
	static public int HasFullHouse(PokerHand ph) {
		if(HasPair(ph)>0){
			return HasThree(ph); 
		}
		return 0;
	...
	static public int HasFour(PokerHand ph) {
		for(int i=0;i<2;i++){
			for(int k=i+1;k<3;k++){
				if(ph.GetCard(i)/4==ph.GetCard(k)/4) {
					for(int j=k+1;j<4;j++){
	...
	static public int HasSgtFlush(PokerHand ph){
		if (HasStraight(ph)>0){
			return HasFlush(ph);
		}
		return 0;
	...
	static public int HasRoyal(PokerHand ph){
		int iRet=HasSgtFlush(ph);
		if (iRet/4==CARD_ACE){
			return iRet;
		}
		return 0;
