...
public class Sorter {
	 ...
	 * 
	 * @param iaToSort The array to sort
	 * @return BblSort({1,6,3,8}) returns {1,3,6,8}
	 */
	static public int[] BblSort(int[] iaToSort){
		int[] iaNewToSort=iaToSort.clone();
    	...
    	iEnd=iaNewToSort.length; //Initialize iEnd so that the loop runs through the entire array
    	...
    	} while (bSwapped); //Early exit if we didn't swap anything on a run through
	...
	static public String BblSort(String sToSort){
    	Boolean bSwapped; //Allows for early exit
    	int i=0;
    	char caWord[]=sToSort.toCharArray(); //Character arrays are easier to work with
    	char cSwap; //For swapping
    	int iEnd; //For efficiency
    	
    	String sRet; //Return value
    	
    	iEnd=sToSort.length(); //Initialize iEnd so that the loop runs through the entire array
    	//The loop doesn't need to run through the entire array every time
    	// because the last element will run to the end of the array every time
    	// and therefore it is sorted. This changed the runtime from:
    	//  T=n^2
	...
	static public String[] BblSort(String saToSort[]){

    	Boolean bSwapped;
    	int i;
    	String[] saWords=saToSort.clone();
    	String sSwap;
    	int iEnd;
    	
    	iEnd=saWords.length; //Initialize iEnd so that the loop runs through the entire array
    	//The loop doesn't need to run through the entire array every time
    	// because the last element will run to the end of the array every time
    	// and therefore it is sorted. This changed the runtime from:
    	//  T=n^2
    ...
    //BblSort({"aaa","zing"},{"d","blah"},{"b","pow"}) returns {"aaa","zing"},{"b","pow"},{"d","blah"}
	static public StrPair[] BblSort(StrPair spaToSort[]){
    	Boolean bSwapped;
    	int i;
    	StrPair[] spaWords=spaToSort.clone();
    	StrPair spSwap;
    	int iEnd;
    ...
    //QwkSort({"aaa","zing"},{"d","blah"},{"b","pow"}) returns {"aaa","zing"},{"b","pow"},{"d","blah"}
	static public StrPair[] QwkSort(StrPair spaToSort[]){
    	//Copy to a new array, don't mess with the original
    	StrPair spaCopy[]=spaToSort.clone();
    ...
    //QwkSort({"aaa","zing"},{"d","blah"},{"b","pow"}) returns nothing, but directly sorts the input array into {"aaa","zing"},{"b","pow"},{"d","blah"}
    //NOTE: This function is private to the class, and is indirectly invoked by the public function QwkSort
	static private void QSControl(StrPair spaToSort[], int iLeft, int iRight){
    	int iIndex;
    ...
    //Sorts a section the input array into the form (less than pivot)+pivot+(more than pivot)
    // and returns the pivot index
    //NOTE: This function is private to the class, and is indirectly invoked by the public function QSControl
	static private int QSSplit(StrPair spaToSort[], int iIndex, int iLeft, int iRight){
    	//Set the pivot value
    	StrPair spPivot=spaToSort[iIndex];
    ...
    //Accessible publicly, this returns the sorted version of the input string, without directly editing the input
    //ex. MergeSort("mergesort") returns "eegmorrst"
	static public String MergeSort(String sToSort){
		char[] caCopy=sToSort.toCharArray(); //Copy the string into a char[]
		MergeSort(caCopy,0,sToSort.length()-1); //Sort the char[]
		//Return the resulting string
		String sRet=new String(caCopy); 
		return sRet;
	}

	//MergeSort controller
	//Not accessible publicly, this performs a recursive MergeSort on the input char[] 
	static private void MergeSort(char[] caToSort,int iLeft,int iRight){
		int iLen=iRight-iLeft+1; //Calculate length
		if (iLen==1){
    ...
    //MergeSort
	//Not publicly accessible, this function takes two sorted input arrays, and turns them into one big sorted one
	//Used in MergeSort controller
    //ex. MSMergeSorter("acegbdfh",0,7) changes it to "abcdefgh"
	static private void MSMergeSorted(char[] caLetters,int iLeft, int iRight){
		int iLen=iRight-iLeft+1; //Length of the entire section to be merged
		...
		for (k=0; k<iLen; k++)
		{
			...
			//If it hasn't moved past the end of either section
				...
				//If the letter at i is earlier in the alphabet than the letter at j
				if (caLetters[i]<caLetters[j]) {
					//Use i
					i++;
					caTemp[k]=caLetters[i-1];
				}
				//If the letter at i is later in the alphabet than the letter at j
				else{
					//Use j
					j++;
					caTemp[k]=caLetters[j-1];
