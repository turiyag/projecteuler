...
public class AVLTree{
	private int _lNodeCount; //Tracks the number of nodes in the tree, also tracks the number of unique words
	private int _lWordCount; //Tracks the number of words in the file
	 ...
	 * @param oElem The element to put into the root node
	 */
	public AVLTree(Object oElem) {
		_nRoot=new AVLNode(oElem);
		_lNodeCount=1;
	}
	/**
	 * Tree with root, the root can be the root of a full tree
	 * This constructor will calculate the height and node count automatically
	 * @param nRoot The root node 
	 */
	public AVLTree(AVLNode nRoot){
		//Make the head and tail point to nothing and make the length 0
		_nRoot=nRoot;
	...
	/**
	 * Sets the root node
	 * @param nRoot The new root node
	 */
	public void SetRoot(AVLNode nRoot){
	 ...
	 * @param sData The string to add to the tree
	 * @return The node containing the newly added word
	 */
	public AVLNode AddWordPos(String sData, int lPos) {
		//Start at the root
		...
		if (nParent==null) {
			...
			WordPosPair wpNew = new WordPosPair(sData, lPos);
			AVLNode nNew = new AVLNode(wpNew);
		...
		while (true) {
			...
			if (sCurr.compareTo(sData)<0) {
				...
				if (nParent.GetRight()!=null) {
					...
					//Make a new node
					AVLNode nNew = new AVLNode(new WordPosPair(sData,lPos),nParent);
	 ...
	 * @param sData The word to search for in the tree
	 * @return The WordPosPair associated to that word, or null if not found
	 */
	public WordPosPair FindWord(String sData){
		//Start at the root
		AVLNode nParent = GetRoot();
		WordPosPair wCurr;
		String sCurr;
	 ...
	 * @param nHead The top of the subtree
	 * @return The height of the subtree, including nHead
	 */
	public int CalcHeight(AVLNode nHead) {
		if (nHead==null) {
		...
		if (nHead.GetLeft()==null) {
			...
			if (nHead.GetRight()==null) {
				...
				int iHLeft=CalcHeight(nHead.GetLeft());
				int iHRight=CalcHeight(nHead.GetRight());
				if (iHLeft>=iHRight) {
					return iHLeft+1;
				} else {
					return iHRight+1;
	...
	/**
	 * Calculate the number of nodes in the subtree of a certain node, including the subtree root
	 * @param nHead The node to count all subnodes of
	 * @return Returns the number of nodes in the subtree, including nHead
	 */
	public int CalcNodeCount(AVLNode nHead) {
		if (nHead==null) {
			//If nHead does not exist, then there is just as much tree 
			// as there is spoon.
			// Reference to The Matrix. Watch it before calling yourself 
	...
	/**
	 * Checks a node (before the node has the correct height) to see if
	 * any AVL rotations are needed to balance the tree
	 * If any rotation is needed, then CheckAVL will call LeftRot and/or RightRot 
	 * to rebalance the tree
	 * NOTE: nGrand's height must not be set yet, CheckAVL will correct the height
	 * @param	nGrand	The node to test balance and set the height of
	 */
	public void CheckAVL(AVLNode nGrand){
		...
		int iOldHeight=nGrand.GetHeight(); //Track the old height of nGrand
	...
	/**
	 * AVL left rotation
	 * @param	nRoot	The current highest node
	 * @param	nPivot	The node that replaces nRoot as the highest node
	 */
	private void LeftRot(AVLNode nRoot, AVLNode nPivot){
		if (nRoot==_nRoot){
			_nRoot=nPivot;
		}
	 ...
	 * @param	nPivot	The node that replaces nRoot as the highest node
	 */
	private void RightRot(AVLNode nRoot, AVLNode nPivot){
		if (nRoot==_nRoot){
			_nRoot=nPivot;
		}
