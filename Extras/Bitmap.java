...
public class Bitmap {
	...
	private int[][] _iaPixels; //Grid of pixels
	private int _iWidth; //Width of bitmap
	private int _iHeight; //Height of bitmap
	...
	public Bitmap(int iWidth,int iHeight) {
	...
	public int GetPixel(int iX,int iY) {
	...
	public void DrawRect(int iX1,int iY1,int iX2,int iY2, int iBorderColor) {
		if (iX1>iX2) {
			int iSwap=iX1;
			iX1=iX2;
			iX2=iSwap;
	...
	public void DrawRect(int iX1,int iY1,int iX2,int iY2, int iBorderColor,int iFillColor) {
		if (iX1>iX2) {
			int iSwap=iX1;
			iX1=iX2;
			iX2=iSwap;
	...
	//Draw a line
	public void DrawLine(int iX1,int iY1,int iX2,int iY2, int iColor) {
		//Width/Height
		int iWidth = iX2-iX1;
		int iHeight = iY2-iY1;
		//Absolute values of width/height
		int iAbsWidth = Math.abs(iWidth);
		int iAbsHeight = Math.abs(iHeight);
		
		//Directions for X and Y
		int iXInc;
		int iYInc;
		//Length of line, in pixels
		int iLen=Math.max(iAbsWidth, iAbsHeight);
	...
	public void WriteImage(String sPath) {
		//Open the file
		FileOutputStream fos=OpenFileOutStream(sPath);
		try {
			//Calculate the image data length
			int iDataLen = Align4(_iWidth*3)*_iHeight;
			//Calculate the total file length, data+header
			int iFileLen = iDataLen + 0x36;
			//Write the header
			byte[] caBM=new byte[2];
			caBM[0]=0x42; //'B'
			caBM[1]=0x4D; //'M'
			fos.write(caBM); //BM
			...
			fos.write(BigEndianBytes(0x00180001),0,4); //Reserved = 1 and bit-depth = 0x18 = 24
			...
			fos.write(BigEndianBytes(0),0,4); //# of colors in palette = 0 (non-indexed)
			fos.write(BigEndianBytes(0),0,4); //# of important indexed colors = 0 (non-indexed)
	...
	public byte[] BigEndianBytes(int i) {
		byte[] ba = new byte[4];
		ba[0]=(byte)(i & 0xFF);
	...
	public static FileOutputStream OpenFileOutStream(String sPath) {
		try {
	...
	public void WritePixels(FileOutputStream fos) {
		int iLineByteWidth=Align4(_iWidth*3); //Length of a line of the image in bytes
		byte[] baImgData=new byte[iLineByteWidth*_iHeight]; //Image data
		byte[] baPixel=null; //Array of bytes to store pixel data
		int iPadBytes = iLineByteWidth-(_iWidth*3); //Calculate pad bytes needed to align each row to 4
		try {
